...
def run_test_largest_number():
    ...
    # Test 4
    expected = 1337
    answer = largest_number(([6, 9, 69], [4, 20, 420], [13, 37, 1337]))
    print('Expected and actual are:', expected, answer)
...
def run_test_largest_negative_number():
    ...
    # Test 1
    expected = -4
    answer = largest_negative_number(([-6, 9, 69], [-4, 20, -420], [13, -37, 1337]))
    print('Expected and actual are:', expected, answer)
...
def largest_negative_number(seq_seq):
    ...
    for k in range(len(seq_seq)):
        for j in range(len(seq_seq[k])):
            if (seq_seq[k])[j] >  highest and (seq_seq[k])[j] < 0:
                highest = (seq_seq[k])[j]
    if highest == -sys.maxsize + 1:
        return None
    else:
...
def first_is_elsewhere_too(seq_seq):
    ...
    for j in range(len(seq_seq[0])):
        initial_int = (seq_seq[0])[j]
        for k in range(len(seq_seq) -1 ):
            for l in range(len(seq_seq[k + 1])):
                int_in_list = (seq_seq[k + 1][l])
                if initial_int == int_in_list:
                    return True
    return False
